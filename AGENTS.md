...
## Testing Xenon Agent Interactive Mode with tmux

To test Xenon Agent's TUI in a controlled terminal environment:

```bash
# Create tmux session with specific dimensions
tmux new-session -d -s xenon-agent-test -x 80 -y 24

# Start Xenon Agent from source
tmux send-keys -t xenon-agent-test "cd /path/to/xenon-agent && ./xenon-agent.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t xenon-agent-test -p

# Send input
tmux send-keys -t xenon-agent-test "your prompt here" Enter

# Send special keys
tmux send-keys -t xenon-agent-test Escape
tmux send-keys -t xenon-agent-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t xenon-agent-test
```
...
- **Internal changes (from issues)**: `Fixed foo bar ([#123](https://github.com/Istiyaq-Khan/xenon-agent/issues/123))`
- **External contributions**: `Added feature X ([#456](https://github.com/Istiyaq-Khan/xenon-agent/pull/456) by [@username](https://github.com/username))`
