...
import type { XenonTeam } from "../../../core/xenon-inference-auth.js";
...
export class XenonTeamSelectorComponent extends Container implements Focusable {
	private readonly searchInput: MenuSearchInput;
	private readonly listContainer: Container;
	private readonly allOptions: XenonTeamOption[];
	private filteredOptions: XenonTeamOption[];
	...
	constructor(
		...
		private readonly onSelect: (team: XenonTeam | null) => void,
		...
		const panel = new MenuPanel({
			title: "Xenon Team",
			subtitle: "Choose which account pays for Xenon Inference usage.",
		});
		this.addChild(panel);
	...
	private getSearchText(option: XenonTeamOption): string {
	...
	private getPrimary(option: XenonTeamOption): string {
		return option.team?.name ?? "Personal";
	}

	private getSecondary(option: XenonTeamOption): string {
		if (!option.team) {
			return "personal account";
		}
	...
	private getMeta(option: XenonTeamOption): string {
