...
const UPDATE_RESTART_MARKER =
	"<xenon_agent_update_interrupted>\n" +
	"Xenon Agent was updated and intentionally interrupted this session. Continue from the saved transcript and restored tool/kernel state. Any running model, tool, bash, or child-agent work may have been partially completed.\n" +
	"</xenon_agent_update_interrupted>";
...
export class AgentDaemon {
	...
	async start(): Promise<void> {
		...
		this.log(`Xenon Agent daemon listening on ${this.socketPath}`);
		...
		try {
			...
			if (this.options.worker && client.authenticated !== true) {
				...
				if (
					...
					clearParsedAdmission();
					this.log(`worker_auth failed: received invalid or malformed payload: ${JSON.stringify(parsed)}`);
				...
				try {
					...
					this.log(
						`worker_auth rejected claim: ${error instanceof Error ? error.message : String(error)} for claim ${JSON.stringify(claim)}`,
					);
