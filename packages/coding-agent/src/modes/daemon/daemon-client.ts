...
export class DaemonSocketClosedError extends Error {
	constructor(
		...
		super(
			`Connection to the Xenon Agent daemon closed.${reasonDetails}${causeDetails} ${daemonEndpointDetails(socketPath)}`,
		);
...
export class DaemonCapabilityUnavailableError extends Error {
	constructor(
		...
		super(
			capability
				? `The running Xenon Agent daemon does not support ${capability}.`
				: `The running Xenon Agent daemon does not support ${command}.`,
		);
...
export class DaemonClient {
	...
	async waitForHello(timeoutMs = 3000): Promise<DaemonHello> {
		...
		if (!this.socket || this.socket.destroyed) {
			throw new Error(
				`Cannot wait for the Xenon Agent daemon handshake because the daemon is not connected. ${daemonEndpointDetails(this.socketPath)}`,
			);
		}
	...
	async connect(timeoutMs = 3000): Promise<void> {
		if (this.socket) {
			throw new Error(`Xenon Agent daemon client is already connected. ${daemonEndpointDetails(this.socketPath)}`);
		}
	...
	async request(
		...
		if (!this.socket || this.socket.destroyed) {
			throw new Error(
				`Cannot send daemon command "${command.type}" because the Xenon Agent daemon is not connected. ${daemonEndpointDetails(this.socketPath)}`,
			);
		}
	...
	close(): void {
		...
		this.rejectAll(
			new Error(
				`Xenon Agent daemon client closed before the operation completed. ${daemonEndpointDetails(this.socketPath)}`,
			),
		);
		this.socket?.end();
		this.socket?.destroy();
