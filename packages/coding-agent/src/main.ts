...
// `xenon agents` (or `xenon-agent agents`) opens the agents view directly.
...
const STARTUP_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		...
		return `A background service from a different Xenon Agent version is running with ${count} busy ${noun}. Stopping it will terminate ${pronoun}.`;
	},
	unlistableDetail:
		"A background service from a different Xenon Agent version is running and its sessions could not be listed. Stopping it may terminate active sessions.",
...
async function prepareRuntimeServices(options: {
	...
	const authStorage = AuthStorage.create(join(effectiveAgentDir, "auth.json"), {
		useXenonCliConfig: effectiveAgentDir === options.agentDir,
	});
