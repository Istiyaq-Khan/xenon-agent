...
import asyncio
import os as _xenon_agent_os

_xenon_agent_os.environ["NO_COLOR"] = "1"
_xenon_agent_os = _xenon_agent_os
`.trim();

const RLM_BOOTSTRAP_RUNTIME_CODE = `
try:
    import rlm as _xenon_agent_rlm_module
    rlm = _xenon_agent_rlm_module.rlm
    bash = _xenon_agent_rlm_module.bash
    import rlm.mcp as mcp
except Exception as _xenon_agent_rlm_error:
    _XENON_AGENT_RLM_IMPORT_ERROR = str(_xenon_agent_rlm_error)
    _XENON_AGENT_RLM_IMPORT_ERROR = _XENON_AGENT_RLM_IMPORT_ERROR

    class _XenonAgentMissingRlm:
        def _raise_missing(self):
            raise RuntimeError(
                "xenon-agent-runtime is not installed in this kernel. "
                "Remove ~/.xenon-agent/kernel-venv so xenon-agent can rebuild it, or set "
                "XENON_AGENT_KERNEL_PYTHON to a kernel environment with xenon-agent-runtime installed. "
                f"Import error: {_XENON_AGENT_RLM_IMPORT_ERROR}"
            )
    ...
    _XenonAgentMissingRlm = _XenonAgentMissingRlm
    rlm = _XenonAgentMissingRlm()
...
import importlib as _xenon_agent_importlib
import inspect as _xenon_agent_inspect
import sys as _xenon_agent_sys
import types as _xenon_agent_types

class _XenonAgentCallableSkillModule(_xenon_agent_types.ModuleType):
    async def __call__(self, *args, **kwargs):
        result = self.run(*args, **kwargs)
        if _xenon_agent_inspect.isawaitable(result):
            return await result
        return result

_XenonAgentCallableSkillModule = _XenonAgentCallableSkillModule

class _XenonAgentUnavailableSkill:
    def __init__(self, name, error):
        self.__name__ = name
        self._xenon_agent_import_error = error
        self.__doc__ = f"Python skill {name} is unavailable: {error}"

    async def run(self, *args, **kwargs):
        raise RuntimeError(
            f"Python skill {self.__name__} is unavailable in this kernel. "
            f"Import error: {self._xenon_agent_import_error}"
        )
    ...
    def __repr__(self):
        return f"<unavailable Python skill {self.__name__!r}: {self._xenon_agent_import_error}>"

_XenonAgentUnavailableSkill = _XenonAgentUnavailableSkill

def _xenon_agent_wrap_skill_module(module):
    run = getattr(module, "run", None)
    if not callable(run):
        return module
    if isinstance(module, _XenonAgentCallableSkillModule):
        return module
    wrapped = _XenonAgentCallableSkillModule(module.__name__)
    wrapped.__dict__.update(module.__dict__)
    try:
        wrapped.__signature__ = _xenon_agent_inspect.signature(run)
    except Exception:
        pass
    doc = getattr(run, "__doc__", None)
    if doc:
        wrapped.__doc__ = doc
    _xenon_agent_sys.modules[module.__name__] = wrapped
    return wrapped

_xenon_agent_wrap_skill_module = _xenon_agent_wrap_skill_module
_XENON_AGENT_SKILL_IMPORT_ERRORS = {}
_XENON_AGENT_SKILL_IMPORT_ERRORS = _XENON_AGENT_SKILL_IMPORT_ERRORS

for _xenon_agent_skill_name in ${JSON.stringify(importNames)}:
    try:
        globals()[_xenon_agent_skill_name] = _xenon_agent_wrap_skill_module(
            _xenon_agent_importlib.import_module(_xenon_agent_skill_name)
        )
    except Exception as _xenon_agent_skill_error:
        _XENON_AGENT_SKILL_IMPORT_ERRORS[_xenon_agent_skill_name] = str(_xenon_agent_skill_error)
        globals()[_xenon_agent_skill_name] = _XenonAgentUnavailableSkill(
            _xenon_agent_skill_name,
            str(_xenon_agent_skill_error),
        )
`.trim();
}
...
export interface IpythonToolOptions {
	/** Python override. Must have xenon-agent-runtime installed. */
	python?: string;
	...
	private async startKernel(signal?: AbortSignal): Promise<KernelClient> {
		...
		try {
			...
			const m = new ReplKernelManager({
				...
				env: {
					...this.options?.env,
					...(shellPath ? { XENON_AGENT_BASH_SHELL: shellPath } : {}),
					...(commandPrefix
						? {
								XENON_AGENT_BASH_COMMAND_PREFIX: commandPrefix,
							}
						: {}),
				},
