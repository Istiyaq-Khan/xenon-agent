...
export const REFINEMENT_CUSTOM_TYPE = "xenon-agent.refinement";
export const LEGACY_REFINEMENT_CUSTOM_TYPE = "xenon-agent.refinement";
...
const REFINEMENT_SYSTEM_PROMPT = `You are Xenon Agent's /refine continual harness subsystem.
...
skills, and subagent specs that lets Xenon Agent improve reusable behavior
...
- The default editable continual harness store is local to the current Xenon Agent session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Xenon Agent's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Xenon Agent session. Global continual harness entries persist across Xenon Agent sessions.",
...
export async function planRefinement(
	...
	const scopeInstruction = options.global
		? "Requested refinement scope: global. Only propose stable cross-session continual harness edits, durable user preferences, reusable skills/subagents, or explicitly project-qualified facts that should affect future Xenon Agent sessions. Do not persist session-only progress, temporary blockers, or current-run coordination globally."
		: "Requested refinement scope: local. Prefer local continual harness edits for current task progress, temporary blockers, current-run coordination, and project facts that are not clearly reusable across Xenon Agent sessions. Global entries in the overview are read-only context: do not propose update or delete edits for them; create a local entry instead if an override is needed.";
	const userPrompt = [
