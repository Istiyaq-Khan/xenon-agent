...
import { resolveXenonInferencePostLoginModelAction } from "../src/core/xenon-inference-model-selection.js";
...
describe("Xenon Inference post-login model selection", () => {
	test("selects GLM 5.2 as a fallback and opens the model picker", () => {
		const action = resolveXenonInferencePostLoginModelAction(
			{
				status: "success",
	...
	test("does nothing when login is cancelled", () => {
		const action = resolveXenonInferencePostLoginModelAction({ status: "cancelled" }, undefined, registry);
