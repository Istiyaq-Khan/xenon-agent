 ...
 *     XENON_AGENT_INTERNAL_DAEMON_WORKER=1 so the auto-spawned supervisor
...
describe("Real-process serializedRefine — JSON mode", () => {
	it("daemon supervisor scrubs inherited worker env, checkpoint applies refine_complete before agent_end", async () => {
		const root = mkdtempSync(join(tmpdir(), "xenon-agent-process-json-refine-"));
		tempRoots.add(root);
		...
		const result = await runCli(
			...
			],
			{
				agentDir,
				environment: {
					XENON_AGENT_TEST_EVENT_LOG: eventLogPath,
