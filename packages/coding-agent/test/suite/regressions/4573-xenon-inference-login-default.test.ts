...
import { findInitialModel, XENON_INFERENCE_DEFAULT_MODEL_ID } from "../../../src/core/model-resolver.js";
...
describe("ENG-4573 Xenon Inference login default", () => {
	...
	test("persists GLM 5.2 so the next process starts with a valid model", async () => {
		const harness = await createHarness({
			...
			models: [{ id: XENON_INFERENCE_DEFAULT_MODEL_ID, name: "GLM 5.2" }],
		});
		harnesses.push(harness);
		const agentDir = join(harness.tempDir, "agent");
		const settings = SettingsManager.create(harness.tempDir, agentDir);

		settings.setDefaultModelAndProvider(XENON_INFERENCE_PROVIDER_ID, XENON_INFERENCE_DEFAULT_MODEL_ID);
		await settings.flush();

		const restartedSettings = SettingsManager.create(harness.tempDir, agentDir);
		expect(restartedSettings.getDefaultProvider()).toBe(XENON_INFERENCE_PROVIDER_ID);
		expect(restartedSettings.getDefaultModel()).toBe(XENON_INFERENCE_DEFAULT_MODEL_ID);
		...
		expect(initial.model?.provider).toBe(XENON_INFERENCE_PROVIDER_ID);
		expect(initial.model?.id).toBe(XENON_INFERENCE_DEFAULT_MODEL_ID);
	});
	...
	test("refreshes authenticated models before selecting and persisting GLM 5.2", async () => {
		const fallbackModel: AgentConnectionModel = {
			id: XENON_INFERENCE_DEFAULT_MODEL_ID,
			name: "GLM 5.2",
