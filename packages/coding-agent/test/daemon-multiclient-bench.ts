...
function socketPathForRun(): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\xenon-agent-multiclient-bench-${process.pid}-${socketCounter}`;
	}
	return `/tmp/xenon-agent-multiclient-${process.pid}-${socketCounter}.sock`;
}
...
const streamCorpus = repeatedText(STREAM_CHARS, "Xenon Agent daemon multi-client streaming benchmark. ");
...
function createAttachResponseForMessages(
	...
	const state = {
		...
		sessionDir: "/tmp/xenon-agent-benchmark/sessions",
		...
		for (let offset = 0, sequence = 1; offset < streamCorpus.length; sequence++) {
			...
			const compact = createCompactAssistantDelta({
				...
				meta: {
					...
					protocol: { name: "xenon-agent.daemon", version: 2 },
					activeSessionId: "active-benchmark",
					sequence,
...
async function runAttachV2(
	...
	const cacheRoot = await mkdtemp(join(tmpdir(), "xenon-agent-attach-v2-bench-"));
...
async function loadSessionFixture(sessionFile: string): Promise<{
	...
	const tempDir = await mkdtemp(join(tmpdir(), "xenon-agent-session-bench-"));
...
function printResults(
	...
	console.log("Xenon Agent daemon multi-client benchmark");
	console.log(details);
	console.table(
		results.map((result) => ({
...
function printSessionLoad(sessionFile: string, result: SessionLoadResult): void {
	console.log("Xenon Agent real-session benchmark");
...
async function generateSessionFixture(sizeMiB: number): Promise<GeneratedSessionFixture> {
	const directory = await mkdtemp(join(tmpdir(), `xenon-agent-generated-${sizeMiB}mib-`));
