...
import { XenonOnboardingSplashComponent } from "../src/modes/interactive/components/xenon-onboarding-splash.js";
import { initTheme } from "../src/modes/interactive/theme/theme.js";
import { XENON_BUTTERFLY_LOGO } from "../src/themes/xenon-logo.js";

describe("XenonOnboardingSplashComponent", () => {
	beforeAll(() => {
		initTheme("dark");
	});
	...
	it("renders a minimal first-run login action", () => {
		const component = new XenonOnboardingSplashComponent(
			() => {},
			() => {},
			{ getRows: () => 36 },
		);
		...
		expect(output).not.toContain("Login with Xenon Intellect");
		...
		expect(output).not.toContain("A coding agent connected to Xenon Intellect.");
		expect(output).not.toContain("Continue with Xenon Intellect");
		...
		expect(output).not.toContain("Start with your Xenon Intellect account.");
		expect(output).not.toContain("Log in with Xenon Intellect");
		expect(output).not.toContain("→");
		expect(output).not.toContain("Use a subscription");
		expect(output).not.toContain("Use an API key");
		expect(output).toContain(XENON_BUTTERFLY_LOGO.split("\n")[0].trim());
		for (const line of lines) {
			expect(visibleWidth(line)).toBeLessThanOrEqual(100);
		}
	});

	it("starts Xenon login on confirm", () => {
		let selected = false;
		const component = new XenonOnboardingSplashComponent(
			() => {
				selected = true;
	...
	it("shows progress and ignores input while onboarding advances", () => {
		...
		const component = new XenonOnboardingSplashComponent(onSelect, onCancel, { getRows: () => 36 });
