...
Xenon Intellect aggregates GPU capacity across providers: rent single GPU pods on demand or deploy multi-node clusters, with persistent network-attached storage.

Live docs: `cli-reference/check-gpu-availability.md`, `cli-reference/provision-gpu.md`, `cli-reference/managing-disks.md`, `tutorials-multi-node-cluster/deploy-multi-node.md`, `tutorials-storage/create-persistent-storage.md` under https://docs.xenonintellect.ai/

## GPU Availability

```bash
xenon availability list                        # all GPUs with pricing
xenon availability list --gpu-type H100_80GB   # filter by GPU type
```

## Pods (single instances)

```bash
xenon config set-ssh-key-path   # one-time: SSH key for pod access (generate on the dashboard profile page)
xenon pods create               # interactive provisioning from availability data
xenon pods list
xenon pods ssh <pod-id>
```
...
Most provisioning commands accept a `--team-id` flag; the REST API equivalent is the `X-Xenon-Team-ID` header.
