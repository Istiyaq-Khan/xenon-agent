> Xenon Agent can create themes. Ask it to build one for your setup.
...
Xenon Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.xenon-agent/themes/*.json`
- Project: `.xenon-agent/themes/*.json`
...
On first run, Xenon Agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.xenon-agent/themes
vim ~/.xenon-agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Istiyaq-Khan/xenon-agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Xenon Agent reloads it automatically for immediate visual feedback.
...
Xenon Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Xenon Agent falls back to the nearest approximation.
