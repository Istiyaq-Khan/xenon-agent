...
Xenon Agent supports subscription-based providers via OAuth and API key providers via environment variables or the auth file. Its built-in model catalog is updated with each Xenon Agent release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.xenon-agent/auth.json` and auto-refresh when expired.
...
Store credentials in `~/.xenon-agent/auth.json`:
...
```bash
xenon --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
xenon --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
xenon --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal Xenon Agent usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
xenon --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Xenon Agent automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
