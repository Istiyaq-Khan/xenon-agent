...
```
~/.xenon-agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.xenon-agent/sessions/`.

Xenon Agent also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Xenon Agent uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Xenon Agent AI)
...
### Extended Message Types (from Xenon Agent Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Xenon Agent (legacy field name)
