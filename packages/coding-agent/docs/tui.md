> Xenon Agent can create TUI components. Ask it to build one for your use case.
...
Xenon Agent shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Xenon Agent is streaming a response.
...
// Restore Xenon Agent's default spinner
ctx.ui.setWorkingIndicator();
```
