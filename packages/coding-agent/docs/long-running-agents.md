...
Xenon Agent combines daemon-backed session workers with persistent state, scheduled prompts, direct agent messaging, goals, and bounded autonomous continuations. These features serve different purposes but share the same session and worker runtime.
...
```bash
xenon-agent list
xenon-agent attach <agent>
```

Other lifecycle commands are:

```bash
xenon-agent agents                  # Open the agents view
xenon-agent rename <agent> <name>   # Give an agent a stable readable name
xenon-agent stop <agent>            # Stop one agent
xenon-agent status                  # Inspect background services
xenon-agent doctor [--fix]          # Diagnose or repair service state
xenon-agent shutdown [--force]      # Stop all agents and services
```
...
```bash
xenon-agent send <agent> "Please verify the latest migration"
```
...
Xenon Agent has three related scheduling surfaces:
...
| `xenon schedule` | User or automation | General one-time or cron prompts targeted at an agent. |
...
```bash
xenon-agent schedule add worker "in 30m" -- "Check the benchmark result"
xenon-agent schedule add worker "0 9 * * 1-5" -- "Review open work"
xenon-agent schedule list --all
xenon-agent schedule cancel <job-id>
```
...
Autonomous mode is a bounded host policy for runs where no human input is expected. Xenon Agent adds follow-up continuations until configured quality gates pass or a continuation, turn, token, or wall-clock limit is reached.
...
Autonomous mode supports limits for continuations, assistant turns, tokens, and wall-clock duration. Gate commands run before the session may finish; a failed gate returns its bounded output to the agent for another attempt. Xenon Agent avoids rerunning the same failed gate when the workspace has not changed.
...
Automatic compaction handles context growth during long tasks. On overflow or near the configured threshold, Xenon Agent summarizes older messages, retains recent context, and continues. The Python kernel persists through compaction, so variables, imports, helper functions, and task state remain available.
