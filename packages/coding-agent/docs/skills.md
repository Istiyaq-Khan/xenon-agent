> Xenon Agent can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that Xenon Agent loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

Xenon Agent implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent Python kernel.
...
- [Creating Skills with Xenon Agent](#creating-skills-with-xenon-agent)
...
- Built-in: `skills/` shipped with the package (lowest precedence)

Discovery rules:
- In `~/.xenon-agent/skills/` and `.xenon-agent/skills/`, direct root `.md` files are discovered as individual skills
...
Xenon Agent ships with built-in skills that load by default:
...
```bash
export XENON_AGENT_WEBSEARCH_TIMEOUT=45
export XENON_AGENT_WEBSEARCH_NUM_RESULTS=5
```
...
```python
print(await websearch("latest Xenon Agent release"))
```
...
For project-level Claude Code skills, add to `.xenon-agent/settings.json`:
...
1. At startup, Xenon Agent scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, Xenon Agent exposes `web_search` in the Python REPL. If the module defines `run()`, the module is wrapped as an async callable:

```python
await web_search("xenon agent skills")
await web_search.run("xenon agent skills")
help(web_search)
```

Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.xenon-agent/kernel-venv`; set `XENON_AGENT_KERNEL_VENV` to override it. If `pyproject.toml` changes, Xenon Agent rebuilds the kernel venv so dependency changes are picked up.

If you set `XENON_AGENT_KERNEL_PYTHON`, Xenon Agent does not install packages into that environment. The Python must already have a current `xenon-agent-runtime` and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
```python
await web_search("xenon agent")
!web_search "xenon agent" --limit 3
```

## Creating Skills with Xenon Agent

Xenon Agent ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.xenon-agent/skills/release-audit. It should expose
...
1. **Scope:** use `.xenon-agent/skills/<name>/` for a project skill committed with the repository, or `~/.xenon-agent/skills/<name>/` for a personal skill.
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh Xenon Agent session after adding a Python-backed skill so kernel setup can install and import the package.
...
Xenon Agent validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
