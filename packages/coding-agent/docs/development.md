...
Xenon Agent requires Node.js 22.8.0 or newer.
...
Xenon Agent is the product, public CLI, and repository name. The monorepo still retains inherited `@earendil-works/pi-*` npm workspace names, a source-package `pi` bin entry, the `pi` package manifest key, and some `PI_*` compatibility environment variables.

## Local Configuration

User configuration lives under `~/.xenon-agent/`. Project-local settings, prompts, themes, extensions, skills, and system-prompt files live under `.xenon-agent/` in the project root. Override the user config directory with `XENON_AGENT_CODING_AGENT_DIR` and the session directory with `XENON_AGENT_SESSION_DIR`.
...
Xenon Agent runs from source and Node.js package output. Always use `src/config.ts` helpers for package assets:
...
Daemon, worker, client, and provider diagnostic logs live under `~/.xenon-agent/logs/`.
