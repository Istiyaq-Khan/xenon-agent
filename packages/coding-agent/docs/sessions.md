...
Xenon Agent saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.xenon-agent/sessions/`. Each session is a JSONL file with a tree structure.

```bash
xenon --continue          # Continue the most recent session
xenon --resume [path|id]  # Browse past sessions or resume one directly
xenon --no-session        # Ephemeral mode; do not save
xenon --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `xenon --resume` opens the same picker at startup, and `xenon --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `xenon --resume -- "continue this work"`.
...
When available, Xenon Agent uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `xenon --resume`.
...
When `/tree` switches away from one branch to another, Xenon Agent can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
