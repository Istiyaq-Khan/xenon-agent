...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Xenon Agent to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.xenon-agent/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
xenon -c                  # Continue most recent session
xenon -r [path|id]        # Browse sessions or resume one directly
xenon --no-session        # Ephemeral mode; do not save
xenon --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `xenon agents`, `xenon list`, or `xenon attach <agent>` to find and reattach to running work. `xenon stop <agent>` stops one root agent, while `xenon shutdown` stops all workers and the local supervisor.
...
- `~/.xenon-agent/AGENTS.md` for global instructions
...
- `.xenon-agent/SYSTEM.md` for a project
- `~/.xenon-agent/SYSTEM.md` globally
...
xenon attach <agent>
xenon stop <agent>
xenon rename <agent> <name>
xenon send <agent> <message>
xenon schedule <list|add|cancel>
xenon status
xenon doctor [--fix]
xenon shutdown [--force]
...
See [Xenon Agent Packages](packages.md) for package sources and security notes.
...
Use `xenon session export <file> [output]` to export a session to HTML.
...
```bash
xenon --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `XENON_AGENT_CODING_AGENT_DIR` | Override config directory; default is `~/.xenon-agent` |
...
Xenon Agent keeps the model-facing tool surface small while making the Python REPL runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and Xenon Agent packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.
