> Xenon Agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Xenon Agent's capabilities. Use it to embed Xenon Agent in other applications, build custom interfaces, or integrate with automated workflows.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.xenon-agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.xenon-agent/extensions/`)
- Project skills:
  - `.xenon-agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.xenon-agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.xenon-agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.xenon-agent/auth.json and ~/.xenon-agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
- If you omit `tools`, Xenon Agent automatically creates them with the correct `cwd`
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.xenon-agent/extensions/`, `.xenon-agent/extensions/`, and `settings.json` extension sources.
...
1. Global: `~/.xenon-agent/settings.json`
2. Project: `<cwd>/.xenon-agent/settings.json`
