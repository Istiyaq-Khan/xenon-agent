...
Connect external services (Linear, Notion, …) to Xenon Agent over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with Xenon Agent's single-tool design, MCP integrations are **not**
...
Credentials are stored once in `~/.xenon-agent/auth.json` under `mcp:<name>`.
...
agent) or the TUI. Both surfaces update only `~/.xenon-agent/settings.json`:

```bash
xenon mcp add remote --url https://mcp.example.com/mcp --bearer-token-env-var EXAMPLE_TOKEN
xenon mcp add local --cwd /absolute/path --env TOKEN=EXAMPLE_TOKEN -- node server.js --stdio
xenon mcp list
xenon mcp get remote
xenon mcp remove remote
```
...
are environment-variable references. Project `.xenon-agent/settings.json` MCP
...
- Generic MCP connections are kernel-local. Separate Xenon Agent sessions use
  separate connections even when they reference the same user setting.
- A custom `XENON_AGENT_KERNEL_PYTHON` must include the current
  `xenon-agent-runtime` dependencies.
