<h1 align="center">Xenon Agent Core</h1>

<p align="center">
  Stateful agent runtime.
</p>

Release docs use the Xenon Agent package names. The source workspace manifests still keep inherited package names until the namespace migration is complete.

## Workspace Package

```bash
npm install xenon-agent-core
```

## Quick Start

```typescript
import { Agent } from "xenon-agent-core";
import { getModel } from "xenon-agent-ai";
...
```typescript
declare module "xenon-agent-core" {
...
```typescript
import { Agent, streamProxy } from "xenon-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "xenon-agent-core";
