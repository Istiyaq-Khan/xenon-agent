<h1 align="center">Xenon Agent AI</h1>
...
- **OpenAI**
- **Xenon Inference** (OpenAI-compatible API)
...
```bash
npm install xenon-agent-ai
```

TypeBox exports are re-exported from `xenon-agent-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'xenon-agent-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'xenon-agent-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'xenon-agent-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'xenon-agent-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'xenon-agent-ai';
...
```typescript
import { getModel, stream } from 'xenon-agent-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'xenon-agent-ai';
...
```typescript
import { Model, stream } from 'xenon-agent-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Xenon Agent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'xenon-agent-ai';
...
```typescript
import { getModel, complete, Context } from 'xenon-agent-ai';
...
```typescript
import { Context, getModel, complete } from 'xenon-agent-ai';
...
- OAuth login flows are not supported in browser environments. Use the `xenon-agent-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'xenon-agent-ai';
...
```bash
npx xenon-agent-ai login              # interactive provider selection
npx xenon-agent-ai login anthropic    # login to specific provider
npx xenon-agent-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `xenon-agent-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'xenon-agent-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'xenon-agent-ai/oauth';
...
```typescript
import { getModel, complete } from 'xenon-agent-ai';
import { getOAuthApiKey } from 'xenon-agent-ai/oauth';
...
**Xenon Inference**: Uses the OpenAI-compatible API at `https://api.pinference.ai/api/v1`. Set `XENON_API_KEY` or pass an API key explicitly.
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `xenon-agent-ai`
