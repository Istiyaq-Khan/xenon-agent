...
frames="${1:-${XENON_AGENT_SPLASH_PREVIEW_FRAMES:-${XENON_AGENT_SPLASH_PREVIEW_FRAMES:-300}}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/xenon-agent-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	xenon_agent_screen "Installing Xenon Agent" "Downloading Xenon Agent$(xenon_agent_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
