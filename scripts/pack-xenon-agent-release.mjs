...
const defaultBaseUrl = process.env.XENON_AGENT_DOWNLOAD_BASE_URL || process.env.XENON_AGENT_DOWNLOAD_BASE_URL;
const publicPackageName = process.env.XENON_AGENT_PACKAGE_NAME || process.env.XENON_AGENT_PACKAGE_NAME || "xenon-agent";
const publicCommandName = process.env.XENON_AGENT_CMD || process.env.XENON_AGENT_CMD || "xenon-agent";
const releaseChannels = new Set(["stable", "beta"]);

const releasePackages = [
	{ packageDir: "ai", publicName: undefined, artifactName: "xenon-agent-ai" },
	{ packageDir: "tui", publicName: undefined, artifactName: "xenon-agent-tui" },
	{ packageDir: "agent", publicName: undefined, artifactName: "xenon-agent-core" },
...
function parseArgs(args) {
	...
	if (!parsed.baseUrl) {
		throw new Error("--base-url or XENON_AGENT_DOWNLOAD_BASE_URL is required");
	}
...
function printHelp() {
	console.log(`Usage: node scripts/pack-xenon-agent-release.mjs --base-url url [--channel stable|beta] [--version x.y.z] [--out-dir path]

Creates private npm tarballs for R2 distribution:

  <out-dir>/artifacts/xenon-agent-<version>.tgz
  <out-dir>/artifacts/xenon-agent-ai-<version>.tgz
  <out-dir>/artifacts/xenon-agent-core-<version>.tgz
  <out-dir>/artifacts/xenon-agent-tui-<version>.tgz
...
function main() {
	...
	const releaseVersion = args.version || normalizeVersion(process.env.XENON_AGENT_VERSION || process.env.XENON_AGENT_VERSION || cliPackage.version);
