"""Tiny rlm-compatible kernel shim for Xenon Agent."""
...
async def host_request(request_type: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    """Send a typed request to the Xenon Agent host and await its reply.
...
async def run(prompt: str, **kwargs: Any) -> RLMSpawnHandle:
    """Spawn a recursive Xenon Agent child and return once its task is admitted.
