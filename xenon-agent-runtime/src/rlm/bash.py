...
_STATUS_FD = 9
_OUTPUT_FD = 8
_COMPLETION_PREFIX = b"\x1exenon-agent-complete:"
...
def _shell() -> str:
    ...
    override = os.environ.get("XENON_AGENT_BASH_SHELL") or os.environ.get("XENON_AGENT_BASH_SHELL")
    if override:
        if not os.path.isabs(override):
            raise ValueError("XENON_AGENT_BASH_SHELL must be an absolute path")
        return override
    if not _IS_POSIX:
        # Never consult PATH on Windows: a repo-controlled PATH could supply
        # the shell. The host injects XENON_AGENT_BASH_SHELL when one exists.
        raise RuntimeError(
            "bash() needs XENON_AGENT_BASH_SHELL set to the absolute path of a "
    ...
    # always injects XENON_AGENT_BASH_SHELL (an absolute path) when a shell exists.
    shell = shutil.which("bash")
    return shell or "/bin/sh"


def _with_prefix(command: str) -> str:
    prefix = os.environ.get("XENON_AGENT_BASH_COMMAND_PREFIX") or os.environ.get("XENON_AGENT_BASH_COMMAND_PREFIX")
...
def _status_script(command: str, completion_a: str, completion_b: str) -> str:
    ...
    return (
        ...
        f"read -r _xenon_agent_gate <&{_STATUS_FD} || exit 127\n"
        "{\n"
        f"{command}\n"
        f"}} {_OUTPUT_FD}>&- {_STATUS_FD}>&-\n"
        "__xenon_status=$?\n"
        "\\set +x\n"
        f"{emit} '\\036xenon-agent-complete:%s%s\\037' "
        f"'{completion_a}' '{completion_b}' >&{_OUTPUT_FD} || exit \"$__xenon_status\"\n"
        f"{emit} '%s\\n' \"$__xenon_status\" >&{_STATUS_FD}\n"
...
def _record_journal(pid: int, active: bool) -> bool:
    ...
    path = os.environ.get("XENON_AGENT_INTERNAL_ORPHAN_PROCESS_JOURNAL") or os.environ.get("XENON_AGENT_INTERNAL_ORPHAN_PROCESS_JOURNAL")
    owner = os.environ.get("XENON_AGENT_KERNEL_OWNER_PID") or os.environ.get("XENON_AGENT_KERNEL_OWNER_PID")
